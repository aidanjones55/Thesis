...
with open('final_encoding.pickle', 'rb') as handle:
    final_encoding = pickle.load(handle)

print(final_encoding)

hot_1_list = [(k, v) for k, v in final_encoding.items()]
print(hot_1_list)
...
plt.ylabel('Encoded Frequency')
plt.savefig('figures/encodefrequency.jpeg')
